...
public class FacebookAnalytics: CAPPlugin {
    ...
    @objc func logEvent(_ call: CAPPluginCall) {
        ...
        if let valueToSum = call.getDouble("valueToSum") {
            if let params = call.getObject("params") {
                AppEvents.logEvent(AppEvents.Name(event), valueToSum: valueToSum, parameters: JSObjectToFBParams(params))
            } else {
                AppEvents.logEvent(AppEvents.Name(event), valueToSum: valueToSum)
            }
        } else {
            if let params = call.getObject("params") {
                AppEvents.logEvent(AppEvents.Name(event), parameters: JSObjectToFBParams(params))
            } else {
    ...
    @objc func logPurchase(_ call: CAPPluginCall) {
        ...
        AppEvents.logPurchase(amount, currency: currency, parameters: JSObjectToFBParams(params))
        call.resolve()
    }

    @objc func logAddPaymentInfo(_ call: CAPPluginCall) {
        let success = call.getInt("success") ?? 0
        AppEvents.logEvent(.addedPaymentInfo, parameters: [AppEvents.ParameterName("success"): success])
    ...
    @objc func logAddToCart(_ call: CAPPluginCall) {
        ...
        AppEvents.logEvent(.addedToCart, valueToSum: amount, parameters: JSObjectToFBParams(params))
        call.resolve()
    }

    @objc func logCompleteRegistration(_ call: CAPPluginCall) {
        let params = call.getObject("params") ?? [String: String]()
        AppEvents.logEvent(.completedRegistration, parameters: JSObjectToFBParams(params))
        call.resolve()
    }
    ...
    @objc func logInitiatedCheckout(_ call: CAPPluginCall) {
        ...
        AppEvents.logEvent(.initiatedCheckout, valueToSum: amount, parameters: JSObjectToFBParams(params))
        call.resolve()
    }
}
